...
public class ScheduleCenter {
	...
	public static List<TakeRecord> getDayTakeRecordsForScheduleToday(
			Context context, Schedule schedule) {
		return getDayTakeRecordsForSchedule(context, schedule, new Date());
	}

	public static List<TakeRecord> getDayTakeRecordsForSchedule(
			Context context, Schedule schedule, Date date) {
	...
	public static Date getNextUntakeScheduledTime(Context context,
			Schedule schedule) {
		int duration = schedule.getDuration();
		List<Integer> days = schedule.getDays();
			...
			if (days == null || days.contains(time.get(Calendar.DAY_OF_WEEK))) {
				...
				for (Time plan : times) {
					...
					if (!next.before(time.getTime())) {
						// Check take records
						List<TakeRecord> records = getDayTakeRecordsForSchedule(
								context, schedule, time.getTime());
						if (records != null) {
							boolean find = false;
							for (TakeRecord record : records) {
								if (plan.equals(record.getPlannedTime())) { // Has
																			// taken,
																			// check
																			// next
																			// schedule
	...
	public static Map<Date, Conformity> getOverallConformity(
		...
		Map<Date, Conformity> map = new TreeMap<Date, Conformity>();
		Date startDate = null; // Get the earliest create date among all
								// schedules
		for (Schedule schedule : schedules) {
			if (!schedule.isTracking()) {
				continue;
		...
		while (!calendar.after(now)) {
			...
			for (Schedule schedule : schedules) {
				...
				System.out.println("schedule_name: "
						+ schedule.getMedicine().getName() + " Times: "
						+ schedule.getTimes().size());

				int plannedThis = getPlannedCount(schedule, calendar);
				if (plannedThis != 0) {
					planned += plannedThis;
					List<TakeRecord> records = getDayTakeRecordsForSchedule(
							context, schedule, calendar.getTime());

					for (TakeRecord takeRecord : records) {
						System.out.println("name: "
								+ takeRecord.getMedicine().getName()
								+ " TakeTime: " + takeRecord.getTakeTime()
								+ " PlannedTime: "
								+ takeRecord.getPlannedTime());
					}
	...
	/**
	 * Get how many times that is planned to take medicine for the schedule at a
	 * date
	 * 
	 * @param schedule
	 * @param calendar
	...
	private static int getPlannedCount(Schedule schedule, Calendar calendar) {
		if (getEndOfDaysAfter(calendar.getTime(), 1).before(
				schedule.getCreateDate())) {
			// if(schedule.getCreateDate().before(calendar.getTime())) { // Not
			// started
			return 0;
		}
		if (schedule.getDuration() > 0) { // Has duration
			Date end = getEndOfDaysAfter(schedule.getCreateDate(),
		...
		if (days != null) { // Has days
			if (!days.contains(calendar.get(Calendar.DAY_OF_WEEK))) { // No
																		// schedule
																		// that
																		// day
				return 0;
