...
//		 }
	...
	protected void onActivityResult(int arg0, int arg1, Intent arg2) {
		Logger.i(TAG, String.format("Back from call, request=%d, result=%d",
				arg0, arg1));
	...
	public void setContentView(View view, LayoutParams params) {
		...
		mBtnInstructions = (Button) layout
				.findViewById(R.id.banner_instructions_btn);
		mBtnInstructions.setOnClickListener(onInstructionsClick);
		if (view != null) {
	...
	private void alertSecondary() {
		...
		if ("".equals(number)) {
			MyToast.show(BannerActivity.this,
					R.string.msg_secondary_number_empty);
			return;
		}
		doAlert(number);
	}
	...
	private void doAlert(String number) {
		...
		if (makeText) {
			...
			sms.sendTextMessage(number, null,
					"Test message from care coordinator", null, null);
		}
	}
	...
	private void initDialogs() {
		...
		mAlertSecDialog = new AlertDialog.Builder(this)
				...
				.setNeutralButton(R.string.dialog_alert_primary_again,
						new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog,
									int which) {
				...
				.setNegativeButton(R.string.dialog_alert_secondary_again,
						new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog,
									int which) {
	...
	private OnClickListener onAlertClick = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			boolean makeCall = PreferenceUtil
					.getEnableAlertCall(BannerActivity.this);
			boolean makeText = PreferenceUtil
					.getEnableAlertText(BannerActivity.this);

			if (makeCall && makeText) {
				mAlertCfmDialog
						.setMessage(getString(R.string.dialog_alert_cfm_call_text));
			} else if (makeCall) {
				mAlertCfmDialog
						.setMessage(getString(R.string.dialog_alert_cfm_call));
			} else if (makeText) {
				mAlertCfmDialog
						.setMessage(getString(R.string.dialog_alert_cfm_text));
			} else {
				mAlertCfmDialog
						.setMessage(getString(R.string.dialog_alert_cfm_none));
			}
	...
	private OnClickListener onInstructionsClick = new OnClickListener() {
		@Override
		public void onClick(View v) {
